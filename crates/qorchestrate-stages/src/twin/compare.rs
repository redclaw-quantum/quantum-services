...
pub struct TwinCompareStage {
    client: Client,
}

impl TwinCompareStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for TwinCompareStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for TwinCompareStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/qtwin/compare", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
