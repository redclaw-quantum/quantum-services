...
pub struct TwinQecUpdateStage {
    client: Client,
}

impl TwinQecUpdateStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for TwinQecUpdateStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for TwinQecUpdateStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/qtwin/qec-update", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
