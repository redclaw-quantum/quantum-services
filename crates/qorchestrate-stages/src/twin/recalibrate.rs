...
pub struct TwinRecalibrateStage {
    client: Client,
}

impl TwinRecalibrateStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for TwinRecalibrateStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for TwinRecalibrateStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/qtwin/recalibrate", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
