...
pub struct QexploreParetoStage {
    client: Client,
}

impl QexploreParetoStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for QexploreParetoStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for QexploreParetoStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/qexplore/sweep", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
