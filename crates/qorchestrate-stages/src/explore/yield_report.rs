...
pub struct YieldReportBuildStage {
    client: Client,
}

impl YieldReportBuildStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for YieldReportBuildStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for YieldReportBuildStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/freq/yield", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
