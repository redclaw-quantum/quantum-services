...
pub struct QecThresholdStage {
    client: Client,
}

impl QecThresholdStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for QecThresholdStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for QecThresholdStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/qstar/threshold", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
