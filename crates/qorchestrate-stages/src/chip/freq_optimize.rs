...
pub struct FreqOptimizeStage {
    client: Client,
}

impl FreqOptimizeStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for FreqOptimizeStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for FreqOptimizeStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/freq/optimize", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
