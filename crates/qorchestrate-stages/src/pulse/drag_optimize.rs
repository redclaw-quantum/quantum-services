...
pub struct DragOptimizeStage {
    client: Client,
}

impl DragOptimizeStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for DragOptimizeStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for DragOptimizeStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/pulse/drag", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
