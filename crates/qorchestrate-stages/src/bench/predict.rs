...
pub struct BenchPredictStage {
    client: Client,
}

impl BenchPredictStage {
    pub fn new() -> Self {
        Self {
            client: Client::new(),
        }
...
impl Default for BenchPredictStage {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl Stage for BenchPredictStage {
    ...
    async fn execute_raw(&self, input: Value, ctx: &StageContext) -> Result<Value, StageError> {
        let resp = self
            .client
            .post(format!("{}/bench/predict", ctx.quantum_api_url))
            .json(&input)
            .send()
            .await
